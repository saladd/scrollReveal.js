...
/*
  ...
  /____/\___/_/   \____/_/_/_/ |_|\___/|___/\___/\__,_/_(_)_/ /____/    v3.0.0 (+ scrolldirection added by saladd)
                                                         /___/
...
window.scrollReveal = (function( window ){
  ...
  var _requestAnimFrame
    ...
    //update saladd: scroll direction check
    , jq_window = $(window)
    , scroll_pos_old = 0
    , scroll_direction = null //true = down, false = up
    , scroll_direction_old
    , elem_all = []; //store elements for later use (for scroll direction inverting)
    //todo: add config var if you want the scroll direction switch
  ...
  scrollReveal.prototype.defaults = {
    ...
    enter:    'bottom',
    move:     '20px',
    move_neg: '-20px', //for switching the scroll direction (easier than removing and adding 'px')
    move_pos: '20px', //todo: build these (in configfactory) from the 'move' parameter in stead of hard coding them here
    over:     '0.6s',
    wait:     '0s',
    easing:   'ease',
  ...
  scrollReveal.prototype.reveal = function( selector, config ){
    ...
    for ( var i = 0; i < elems.length; i++ ){
      ...
      elem_all.push(elem); //store the elements for use in applyScrollDirection
    }
  ...
  function applyScrollDirection(){ //invert direction when needed
    var i,
    tot = elem_all.length;

    for(var i=0; i<tot; i++){
      if(!scroll_direction){
        elem_all[i].config.move = elem_all[i].config.move_neg;
      } else {
        elem_all[i].config.move = elem_all[i].config.move_pos;
      }
      elem_all[i].styles = self.styleFactory( elem_all[i] );
    }
  };
...
// Private Methods /////////////////////////////////////////////////////////////
  ...
  _handler = function( e ){
    ...
    if ( !self.blocked ){
      ...
      //check for scroll direction //todo: check if you want the scroll direction switch
      var scroll_pos = jq_window.scrollTop();
      scroll_direction = scroll_pos > scroll_pos_old;
      scroll_pos_old = scroll_pos;
      if(scroll_direction_old != scroll_direction) applyScrollDirection();
